...
func NewUdpForward() *UdpForward {
	return &UdpForward{
		...
		ConnectedEvent:    func(addr string) { log.Printf("connected from %v", addr) },
		DisConnectedEvent: func(addr string) { log.Printf("disconnected from %v", addr) },
	}
}

// DoUdpForward
// version: udp版本： udp/udp6
func (_self *UdpForward) DoUdpForward(srcAddr string, destAddr string) error {

	var err error
	_self.SrcAddr, err = net.ResolveUDPAddr("udp", srcAddr)
	if err != nil {
		log.Error("ResolveUDPAddr ", srcAddr, " 出错：", err)
		return err
	}

	_self.DestAddr, err = net.ResolveUDPAddr("udp", destAddr)
	if err != nil {
		log.Error("ResolveUDPAddr ", destAddr, " 出错：", err)
		return err
	}
	...
	if err != nil {
		log.Error("启动UDP监听 ", srcAddr, " 出错：", err)
		return err
	}

	_self.UdpConnsMutex.Lock()
	_self.UdpConns[_self.LClientAddr.String()] = UdpConn{
		udp:        _self.UdpListenerConn,
		lastActive: time.Now(),
	}
...
func (_self *UdpForward) forwardHandler(data []byte, addr *net.UDPAddr) {
	...
	if found {
		...
		if err != nil {
			log.Error("udp-forwader: failed to dial:", err)
			return
		}
